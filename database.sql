...
-- GET MAX EXPENSE WITH DESCRIPTION
SELECT expense_amount, expense_desc FROM expense
WHERE expense_amount = (
   SELECT MAX(expense_amount) FROM expense
);

-- GET MIN EXPENSE WITH DESCRIPTION
SELECT expense_amount, expense_desc FROM expense
WHERE expense_amount = (
   SELECT MIN(expense_amount) FROM expense
);